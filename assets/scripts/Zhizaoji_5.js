...
cc.Class({
    ...
    onLoad () {
        ...
        this._animCtrl  = this.hand.getComponent(cc.Animation);
        this._animState = this._animCtrl.getAnimationState('zzj5');

        this._jihuoanimCtrl = this.jihuoanimal_node.getComponent(cc.Animation);
        this._jihuoanimCtrl_state = this._jihuoanimCtrl.getAnimationState("jihuo5");

        cc.systemEvent.on("TOUCH_JIASU5",this.cancelJiasu.bind(this));
        cc.systemEvent.on("MANCHINE_5_SHENJI",this.machineShengji.bind(this));                  //机器升级通知
        cc.systemEvent.on("MANCHINE_5_JINJIE",this.machineJinjie.bind(this));                   //机器进阶通知
        ...
        cc.systemEvent.on("ZZJ5_INIT",this.zzj_init.bind(this));
        ...
        this.node.on(cc.Node.EventType.TOUCH_START,function(){                                  //点击加速
            ...
            cc.systemEvent.emit("TOUCH_JIASU5");
...
///////////////////////////////////////////
    start () {
        this._jihuoanimCtrl.play("jihuo5");
    },
    ...
    zzj_init:function(){
      ...
      //初始化数据跟界面
      init:function(){
        var lv = require("PlayerManager").zhizaoji_lv[4];                               //制造机器1的等级
        var jiesu = require("PlayerManager").zhizaoji_intensify_lv[4] + JINJIE_LV;                  //制造及1的阶数
        var double_num = this.doubleDamage(jiesu);

        this._damage =  this._BigNum.MulNum( require("ComMain").gameCfgs.machinelevel[lv].damage5 , double_num ) ;          //攻击力
        //this._damage =  require("ComMain").gameCfgs.machinelevel[lv].damage5;          //攻击力
    ...
    //攻击范围
    setFanwei:function(){
        this.fanwei_node.radius = 30 * this._fanwei;                     
    },
