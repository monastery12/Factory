...
cc.Class({
    ...
    properties: {
        ...
        dengji:cc.Label,                                //等级
        starts:cc.Node,                                 //星星
        fanwei_node:cc.CircleCollider,                     //碰撞组建的范围

        skin_1:cc.Sprite,                               //皮肤1
        skin_2:cc.Sprite,                               //皮肤2
        s3:cc.Sprite,
        s4:cc.Sprite,
        // skin_2_2:cc.Sprite,                             //皮肤2_2
    ...
    onLoad () {
        ...
        this._left_animCtrl_state = this._left_animCtrl.getAnimationState("zzj4");

        this._jihuoanimCtrl = this.jihuoanimal_node.getComponent(cc.Animation);
        this._jihuoanimCtrl_state = this._jihuoanimCtrl.getAnimationState("jihuo4");

        cc.systemEvent.on("TOUCH_JIASU2",this.cancelJiasu.bind(this));
        cc.systemEvent.on("RUN",this.run.bind(this));
        cc.systemEvent.on("MANCHINE_2_SHENJI",this.machineShengji.bind(this));                  //机器升级通知
        cc.systemEvent.on("MACHINE_2_JINJIE",this.machineJinjie.bind(this));                   //机器进阶通知
        ...
        cc.systemEvent.on("ZZJ4_INIT",this.zzj_init.bind(this));
        ...
        this.node.on(cc.Node.EventType.TOUCH_START,function(){                                  //点击加速
            ...
            if(self._left_animCtrl_state.isPlaying == false){
                self._left_animCtrl_state.speed = 3;
                self.run();
            }else{
                self._left_animCtrl_state.speed = 3;
            }   
            ...
            cc.systemEvent.emit("TOUCH_JIASU2");
        });
        ...
        cc.loader.loadRes("Images/zhizaoji/machine_5", cc.SpriteAtlas, function (err, atlas) {             //加载图集
            self._assest = atlas;
         });
    ...
    start () {
        ...
        this._jihuoanimCtrl.play("jihuo4");
    },
    ...
    run:function(){
        var self  = this;
        if(this._left_animCtrl_state.isPlaying == false){
            this._left_animCtrl.play("up_down");
        }
    },
  ...
  //初始化数据跟界面
  init:function(){
    var lv = require("PlayerManager").zhizaoji_lv[3];                               //制造机器1的等级 
    var jiesu = require("PlayerManager").zhizaoji_intensify_lv[3] +  JINJIE_LV ;                  //制造及1的阶数

    var double_num = this.doubleDamage(jiesu);

    this._damage =  this._BigNum.MulNum( require("ComMain").gameCfgs.machinelevel[lv].damage4 , double_num ) ;          //攻击力
    //this._damage =  require("ComMain").gameCfgs.machinelevel[lv].damage4;          //攻击力
...
loadSkin:function(){    
    ...
    var str_name_1 = "type_" + self._skin_type.toString();                                          //type_1
    var str_name_2 = "type_" + self._skin_type.toString()+"_" + self._skin_type.toString();         //type_1_1
    // var str_name_3 = "type_" + self._skin_type.toString()+"_" + self._skin_type.toString()+"_"+self._skin_type.toString();      //type_1_1_1

    var sp1 = this._assest.getSpriteFrame(str_name_1);
    var sp2 = this._assest.getSpriteFrame(str_name_2);
    // var sp3 = this._assest.getSpriteFrame(str_name_3);
    /**
     * 加载到资源后
     */
...
//攻击范围
setFanwei:function(){
    this.fanwei_node.radius = 65 * this._fanwei;                     
},
