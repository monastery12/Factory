...
const ACHI_ZZJ1_LV = 14;
const ACHI_ZZJ_JJ = 20;
...
//////////////////////////////////
    ...
    init:function(){
        ...
        this._damage = require("ComMain").gameCfgs.machinelevel[ this._lv ].damage4;         //伤害
    ...
    initYupan:function(){
        ...
        var xiaci_damage =  this._BigNum.MulNum(require("ComMain").gameCfgs.machinelevel[ dengji_lv ].damage4  ,this.doubleDamage(this._jinjie_lv) ) ;   
    ...
    loadSkin:function(){    
        ...
        if(self._assest){
            ...
            var str_name_3 = "type_" + self._skin_type.toString()+"_" + self._skin_type.toString()+"_" + self._skin_type.toString();
         ...
         if(  this._BigNum.ChargeBig(all_coins ,need_coin_num) ){
           ...
           if( next_dengji > need_lv ){
            ...
            this._money_ani.play("sell_money");
             cc.systemEvent.emit("MANCHINE_2_SHENJI");
    ...
    jinjieButton:function(){
        ...
        var need_screw = this.needScrewCount(dengji,next_dengji) ;  
        
        var nowdj = require("PlayerManager").zhizaoji_lv[ZZJ];   //当前等级
        var jjlv = require("PlayerManager").zhizaoji_intensify_lv[ZZJ]+JINJIE_LV;
        var need_lv =  require("ComMain").gameCfgs.machindegree[jjlv].levellimit ; 
        if( nowdj < need_lv){
            messageTips("等级不够，不能进阶");
            return 
        }
        ...
        if( all_screw >= need_screw    ){
            ...
            if( next_dengji >  MAX_JINJIE_LV ){
             ...
             this.init();                                                           //刷新界面
             cc.systemEvent.emit("MACHINE_2_JINJIE");
             cc.systemEvent.emit("UI_CHANGE");
             console.log("进阶成功");
             this.red.active = false;
             cc.systemEvent.emit("RED_CHANGE");
    ...
    needMoneyCount(lv,next_lv){                                     //当前等级跟要升的等级
        ...
        for(var i = 0;i<sub;i++){
            sum = this._BigNum.Add( sum, machinelevel[lv+1+i].consume4 );
        }
        var touxianlv = require("PlayerManager").touxianlv;     //头衔等级
        var touxianjiacheng = require("ComMain").gameCfgs.touxian[touxianlv].jiacheng;
