...
cc.Class({
    ...
    onLoad () {
        ...
        this._animCtrl  = this.yaobai.getComponent(cc.Animation);
        this._animState = this._animCtrl.getAnimationState('yaobai');

        this._jihuoanimCtrl = this.jihuoanimal_node.getComponent(cc.Animation);
        this._jihuoanimCtrl_state = this._jihuoanimCtrl.getAnimationState("jihuo3");

        cc.systemEvent.on("TOUCH_JIASU4",this.cancelJiasu.bind(this));
        cc.systemEvent.on("MANCHINE_4_SHENJI",this.machineShengji.bind(this));                  //机器升级通知
        cc.systemEvent.on("MANCHINE_4_JINJIE",this.machineJinjie.bind(this));                   //机器进阶通知
        ...
        cc.systemEvent.on("ZZJ3_INIT",this.zzj_init.bind(this));
        ...
        this.node.on(cc.Node.EventType.TOUCH_START,function(){                                  //点击加速
            ...
            self._animState.speed = 4;
            cc.systemEvent.emit("TOUCH_JIASU4");
            require("PlayerManager").achievement_jindu[CLICK] ++;
        });

        cc.loader.loadRes("Images/zhizaoji/machine_4", cc.SpriteAtlas, function (err, atlas) {             //加载图集
            self._assest = atlas;
         });
    },
    ...
    start () {
        this._jihuoanimCtrl.play("jihuo3");
    },
  ...
  init:function(){
    ...
    var lv   = require("PlayerManager").zhizaoji_lv[2];                               //制造机器1的等级
    var jiesu = require("PlayerManager").zhizaoji_intensify_lv[2] + JINJIE_LV ;                  //制造及1的阶数

    var double_num = this.doubleDamage(jiesu);

    this._damage =  this._BigNum.MulNum( require("ComMain").gameCfgs.machinelevel[lv].damage3 , double_num ) ;          //攻击力
    //this._damage =  require("ComMain").gameCfgs.machinelevel[lv].damage3;          //攻击力
...
//攻击范围
setFanwei:function(){
    this.fanwei_node.radius = 60 * this._fanwei;                     
},
