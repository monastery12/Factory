...
const ACHI_ZZJ1_LV = 16;
const ACHI_ZZJ_JJ = 22;
...
const JINJIE_LV = 100;
const ZZJ = 5;
const MAX_JINJIE_LV = 120;

const JINJIE = 100;

const SJ_COUNT = 1;                         //成就记录，升级次数
...
//////////////////////////////////
    ...
    init:function(){
        ...
        this._damage = require("ComMain").gameCfgs.machinelevel[ this._lv ].damage6;         //伤害
    ...
    initYupan:function(){
        ...
        var xiaci_damage =  this._BigNum.MulNum(require("ComMain").gameCfgs.machinelevel[ dengji_lv ].damage6  ,this.doubleDamage(this._jinjie_lv) ) ;   
        this.damage_lb_next.string =  this._BigNum.Show(xiaci_damage); 
        this.need_coin.string = "$"+ this._BigNum.Show( this.needMoneyCount(this._lv,dengji_lv) );
    },
    ...
    loadSkin:function(){    
        ...
        if(self._assest){
            ...
            cc.loader.loadRes("Images/zhizaoji/machine_6", cc.SpriteAtlas, function (err, atlas) {             //加载图集
                self._assest = atlas;
                var str_name_1 = "type_" + self._skin_type.toString();
                var str_name_2 = "type_" + self._skin_type.toString()+"_" + self._skin_type.toString();
         ...
         if(this._BigNum.ChargeBig(all_coins ,need_coin_num) ){
           ...
           if( next_dengji > need_lv ){
            ...
            this._money_ani.play("sell_money");
             ...
             cc.systemEvent.emit("MANCHINE_6_SHENJI");
    ...
    jinjieButton:function(){
        ...
        var dengji = playerManager.zhizaoji_intensify_lv[ZZJ] + JINJIE;                          //一号机器的进阶等级
        var next_dengji = dengji + 1;                                                 //下一级
        // var need_screw = require("ComMain").gameCfgs.machindegree[ next_dengji ].consume ;                  //下一级需要消耗的材料数量
        var need_screw = this.needScrewCount(dengji,next_dengji) ;                  //下一级需要消耗的材料数量
        ...
        if( all_screw >= need_screw    ){
             ...
             */
             ...
             this.setStartTexiao();
             this.init();                                                           //刷新界面
             cc.systemEvent.emit("MANCHINE_6_JINJIE");
             cc.systemEvent.emit("UI_CHANGE");
             console.log("进阶成功");
             this.red.active = false;
             cc.systemEvent.emit("RED_CHANGE");
    ...
    needMoneyCount(lv,next_lv){                                     //当前等级跟要升的等级
        ...
        for(var i = 0;i<sub;i++){
            sum = this._BigNum.Add( sum, machinelevel[lv+1+i].consume6 );
        }
